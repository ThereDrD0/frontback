...
function formatReminder(reminder) {
  ...
  return `<small>Напоминание: ${new Date(reminder).toLocaleString()}</small>`;
}
...
function renderNotes() {
  ...
  list.innerHTML = notes
    .map((note) => `<li>${escapeHtml(note.text)}${formatReminder(note.reminder)}</li>`)
    .join("");
}
...
function initNotes() {
  ...
  if (reminderForm.dataset.ready !== "1") {
    reminderForm.dataset.ready = "1";

    reminderForm.addEventListener("submit", (event) => {
      event.preventDefault();

      const text = reminderText.value.trim();
      const reminder = new Date(reminderTime.value).getTime();

      if (!text || !reminderTime.value) {
        return;
      }
